...
export const emailTemplates: EmailTemplates = {
	welcome: (name: string, email: string, token: string) => ({
		subject: 'Welcome to Legacy Venture Limited!',
		html: `
		${emailStyles}
		<div class="container">
			<img src="https://www.legacyventuredltd.vercel.app/logo.png" alt="Legacy Venture Limited Logo" class="logo" />
			<h1>Welcome to Legacy Venture Limited, ${name}!</h1>
			...
			<div class="footer">
				<p>Best regards,<br>The Legacy Venture Limited Team</p>
			</div>
		</div>
		`,
	}),
...
export const sendEmail = async <T extends keyof EmailData>(
	...
	try {
		...
		const mailOptions = {
			from: `"Legacy Venture Limited" <${
				process.env.EMAIL_FROM || 'noreply@legacyventuredltd.vercel.app'
			}>`,
			replyTo: `"Legacy Venture Limited Support" <${
				process.env.EMAIL_FROM || 'noreply@legacyventuredltd.vercel.app'
			}>`,
			to,
			subject,
